...
#     (assure-toi que ANN_morelayers.py est à côté de app.py, avec deep_neural_network & predict)
...
def auto_read_csv(file_storage) -> pd.DataFrame:
    """Tente lecture CSV flexible (détecte séparateur)."""
    try:
        file_storage.seek(0)
...
def map_columns_any_csv(df: pd.DataFrame) -> pd.DataFrame:
    """Mappe automatiquement les noms de colonnes vers SurfaceMoy, NbMaisons, Prixm2Moyen, PrixMoyen."""
    ...
    if missing:
        raise ValueError("Colonnes manquantes dans le CSV : " + ", ".join(CANON[k] for k in missing))
...
def generate_plots(df: pd.DataFrame, preds: np.ndarray, history: np.ndarray, ts: int) -> dict:
    """
    Sauvegarde toutes les figures DANS app.static_folder et renvoie les noms de fichiers.
    """
    out = {}
    ...
    plt.ylabel("Nb de maisons")
    plt.grid(True, alpha=0.3)
    f3 = f"scatter_{ts}.png"
    save_current(f3)
    out["scatter"] = f3

    # 4) scatter prédictions (couleur = prix prédit)
    plt.figure(figsize=(9, 5))
...
def index():
    if request.method == "POST":
        ...
        if not f or not f.filename:
            return render_template("index.html", error="Aucun fichier CSV sélectionné.")

        try:
            df = auto_read_csv(f)
            if df is None or df.empty:
                return render_template("index.html", error="Le CSV est vide ou illisible.")
            ...
            if len(df) < 2:
                return render_template("index.html", error="Pas assez de lignes valides après nettoyage.")

            # Features / target
            X = df[["SurfaceMoy", "NbMaisons", "Prixm2Moyen"]].T.values.astype(np.float64)
            y = df["PrixMoyen"].values.reshape(1, -1).astype(np.float64)

            # Standardisation X (stabilise l'entraînement)
            ...
            # Entraînement (réglages “raisonnables” pour ne pas bloquer)
            ...
            # Prédictions à l'échelle d'origine
            y_pred_std = predict(X_std, params)     # (1, m)
            y_pred = y_pred_std * y_std + y_mean    # (1, m)
            y_pred_flat = y_pred.flatten()

            ts = int(time.time())
            plots = generate_plots(df, y_pred_flat, history.flatten(), ts)

            return redirect(url_for("results", **plots))
...
@app.route("/results")
def results():
    # Reçoit via query params : train, prix, scatter, scatter_pred
